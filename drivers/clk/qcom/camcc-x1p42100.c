...
static struct clk_vdd_class *cam_cc_x1p42100_regulators[] = {
	&vdd_mm,
	&vdd_mx,
	&vdd_mxc,
};

static struct clk_vdd_class *cam_cc_x1p42100_regulators_1[] = {
	&vdd_mm,
	&vdd_mxc,
};
...
static struct clk_rcg2 cam_cc_bps_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = cam_cc_x1p42100_regulators_1,
		.num_vdd_classes = ARRAY_SIZE(cam_cc_x1p42100_regulators_1),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 200000000,
			[VDD_LOW] = 400000000,
...
static struct clk_rcg2 cam_cc_cphy_rx_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = cam_cc_x1p42100_regulators,
		.num_vdd_classes = ARRAY_SIZE(cam_cc_x1p42100_regulators),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
			[VDD_LOWER] = 400000000,
			[VDD_LOW] = 480000000},
...
static struct qcom_cc_desc cam_cc_x1p42100_desc = {
	...
	.clk_regulators = cam_cc_x1p42100_regulators,
	.num_clk_regulators = ARRAY_SIZE(cam_cc_x1p42100_regulators),
...
static int cam_cc_x1p42100_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &cam_cc_x1p42100_desc);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	ret = qcom_cc_runtime_init(pdev, &cam_cc_x1p42100_desc);
	if (ret)
		return ret;
	...
	ret = qcom_cc_really_probe(&pdev->dev, &cam_cc_x1p42100_desc, regmap);
	if (ret) {
...
static void cam_cc_x1p42100_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &cam_cc_x1p42100_desc);
}

static const struct dev_pm_ops cam_cc_x1p42100_pm_ops = {
...
static struct platform_driver cam_cc_x1p42100_driver = {
	...
	.driver = {
		...
		.sync_state = cam_cc_x1p42100_sync_state,
		.pm = &cam_cc_x1p42100_pm_ops,
	},
};
