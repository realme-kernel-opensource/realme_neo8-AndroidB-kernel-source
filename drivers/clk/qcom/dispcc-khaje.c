...
static struct clk_vdd_class *disp_cc_khaje_regulators[] = {
	&vdd_cx,
};
...
static const struct qcom_reset_map disp_cc_khaje_resets[] = {
...
static const struct qcom_cc_desc disp_cc_khaje_desc = {
	...
	.resets = disp_cc_khaje_resets,
	.num_resets = ARRAY_SIZE(disp_cc_khaje_resets),
	.clk_regulators = disp_cc_khaje_regulators,
	.num_clk_regulators = ARRAY_SIZE(disp_cc_khaje_regulators),
};
...
static int disp_cc_khaje_probe(struct platform_device *pdev)
{
	...
	clk = clk_get(&pdev->dev, "gcc_disp_gpll0_clk");
	if (IS_ERR(clk)) {
		if (PTR_ERR(clk) != -EPROBE_DEFER)
			dev_err(&pdev->dev, "Unable to get gcc_disp_gpll0_clk clock\n");
		return PTR_ERR(clk);
	}
	clk_put(clk);
	...
	ret = qcom_cc_really_probe(&pdev->dev, &disp_cc_khaje_desc, regmap);
	if (ret) {
...
static void disp_cc_khaje_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &disp_cc_khaje_desc);
}
...
static struct platform_driver disp_cc_khaje_driver = {
	...
	.driver = {
		...
		.sync_state = disp_cc_khaje_sync_state,
	},
};
