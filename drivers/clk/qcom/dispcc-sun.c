...
static const struct clk_parent_data disp_cc_parent_data_11_ao[] = {
...
static struct clk_rcg2 disp_cc_sleep_clk_src = {
	...
	.clkr.hw.init = &(const struct clk_init_data) {
		...
		.parent_data = disp_cc_parent_data_11_ao,
		.num_parents = ARRAY_SIZE(disp_cc_parent_data_11_ao),
		.flags = CLK_SET_RATE_PARENT,
		.ops = &clk_rcg2_ops,
	},
};
...
static struct gdsc disp_cc_mdss_core_gdsc = {
	...
	.flags = POLL_CFG_GDSCR | RETAIN_FF_ENABLE | HW_CTRL | SKIP_DIS,
	.supply = "vdd_mm",
};
...
static const struct regmap_config disp_cc_sun_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc disp_cc_sun_desc = {
	.config = &disp_cc_sun_regmap_config,
	.clks = disp_cc_sun_clocks,
	.num_clks = ARRAY_SIZE(disp_cc_sun_clocks),
	.resets = disp_cc_sun_resets,
...
MODULE_DEVICE_TABLE(of, disp_cc_sun_match_table);

static int disp_cc_sun_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &disp_cc_sun_desc);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	ret = qcom_cc_runtime_init(pdev, &disp_cc_sun_desc);
	if (ret)
		return ret;
...
static void disp_cc_sun_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &disp_cc_sun_desc);
}

static const struct dev_pm_ops disp_cc_sun_pm_ops = {
	SET_RUNTIME_PM_OPS(qcom_cc_runtime_suspend, qcom_cc_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
};

static struct platform_driver disp_cc_sun_driver = {
	.probe = disp_cc_sun_probe,
	.driver = {
		.name = "disp_cc-sun",
		.of_match_table = disp_cc_sun_match_table,
		.sync_state = disp_cc_sun_sync_state,
		.pm = &disp_cc_sun_pm_ops,
	},
};
...
static int disp_cc_mx_sun_probe(struct platform_device *pdev)
{
	...
	ret = qcom_cc_really_probe(&pdev->dev, &disp_cc_mx_sun_desc, regmap);
	if (ret) {
...
MODULE_DESCRIPTION("QTI DISP_CC SUN Driver");
MODULE_LICENSE("GPL");
