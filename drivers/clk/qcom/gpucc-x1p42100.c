...
static DEFINE_VDD_REGULATORS(vdd_gmxc, VDD_HIGH_L1 + 1, 1, vdd_corner);

static struct clk_vdd_class *gpu_cc_x1p42100_regulators[] = {
	&vdd_cx,
	&vdd_mx,
	&vdd_gmxc,
};

static struct clk_vdd_class *gpu_cc_x1p42100_regulators_1[] = {
	&vdd_cx,
	&vdd_mx,
};
...
static struct clk_rcg2 gpu_cc_gmu_clk_src = {
	...
	.clkr.vdd_data = {
		.vdd_classes = gpu_cc_x1p42100_regulators_1,
		.num_vdd_classes = ARRAY_SIZE(gpu_cc_x1p42100_regulators_1),
		.num_rate_max = VDD_NUM,
		.rate_max = (unsigned long[VDD_NUM]) {
...
static struct qcom_cc_desc gpu_cc_x1p42100_desc = {
	...
	.clk_regulators = gpu_cc_x1p42100_regulators,
	.num_clk_regulators = ARRAY_SIZE(gpu_cc_x1p42100_regulators),
...
static int gpu_cc_x1p42100_probe(struct platform_device *pdev)
{
	...
	ret = qcom_cc_runtime_init(pdev, &gpu_cc_x1p42100_desc);
	if (ret)
		return ret;
	...
	regmap_update_bits(regmap, 0x9134, BIT(0), BIT(0));
...
static void gpu_cc_x1p42100_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gpu_cc_x1p42100_desc);
}
...
static struct platform_driver gpu_cc_x1p42100_driver = {
	...
	.driver = {
		...
		.sync_state = gpu_cc_x1p42100_sync_state,
	},
};
