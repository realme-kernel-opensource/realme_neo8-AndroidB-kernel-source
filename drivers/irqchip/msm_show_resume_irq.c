...
struct gic_chip_data *gic_data_glb;
...
static atomic_t hib_active = ATOMIC_INIT(0);
...
static void gic_suspend_ds(void *data, struct gic_chip_data *gic_data, int *ret)
{
	int i;
	void __iomem *rdist_base = gic_data_rdist_sgi_base();
	u32 gic_line_nr = min(GIC_LINE_NR, 1020U);

	gic_data_glb = gic_data;

	if (unlikely(!atomic_read(&hib_active)))
		return;
...
static void gic_resume_ds(struct gic_chip_data *gic_data)
{
	int i;
	void __iomem *rdist_base = gic_data_rdist_sgi_base();
