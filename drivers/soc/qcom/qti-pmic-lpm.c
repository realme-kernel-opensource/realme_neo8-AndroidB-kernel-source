...
#define SDAM_INT_TEST_VAL_BIT_LOW	0
#define SDAM_INT_TEST_VAL_BIT_HIGH	BIT(1)
...
static ssize_t pmic_twm_enable_store(const struct class *c,
			...
			const char *buf, size_t count)
{
	...
	if (chip->twm_enable) {
		...
		if (rc < 0) {
			pr_err("PMIC: Failed to write to pmic sdam offset %#x, rc=%ld\n",
							SDAM_INT_REASON_REG, rc);
			return -EINVAL;
		}
		...
		pr_debug("PMIC : Successfully sent Button owner notification to Co-proc\n");
	}

	return count;
}

static ssize_t pmic_twm_enable_show(const struct class *c,
...
static ssize_t pmic_twm_exit_show(const struct class *c,
			const struct class_attribute *attr, char *buf)
{
	struct qti_pmic_lpm *chip = container_of(c, struct qti_pmic_lpm,
						twm_class);
...
static ssize_t pmic_ds_exit_show(const struct class *c,
			const struct class_attribute *attr, char *buf)
{
	struct qti_pmic_lpm *chip = container_of(c, struct qti_pmic_lpm,
						twm_class);
...
static void qti_pmic_lpm_remove(struct platform_device *pdev)
{
...
static void qti_pmic_lpm_syscore_shutdown(void)
{
	...
	if (gchip->twm_enable) {
		...
		do {
			...
			/*
			 * Shutdown callback is called from atomic context,
			 * mdelay is used to add a delay for APPS shutdown
			 * until Co-proc notifies APPS of TWM entry.
			 */
