...
static int __init hab_init(void)
{
	...
	ret = shmem_hab_hypervisor_register();
	if (ret) {
		pr_err("shmem register HYP plug-in failed, ret %d\n", ret);
		return ret;
	}
	virtio_hab_hypervisor_register_post();
	pr_info("Shmem & VirtIO Register succeeds\n");
