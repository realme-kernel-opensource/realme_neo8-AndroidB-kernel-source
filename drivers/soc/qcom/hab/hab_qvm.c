		...
		int vmid_remote, struct hab_device *mmid_device)
{
	...
	*pchan = hab_pchan_alloc(mmid_device, vmid_remote);
	...
	ret = shmem_habhyp_commdev_create_dispatcher(*pchan);
	if (ret < 0)
		goto err;
...
int shmem_hab_hypervisor_register(void)
{
	...
	ret = shmem_hab_hypervisor_register_os();
...
{
	shmem_hab_hypervisor_unregister_os();
}

int shmem_hab_hypervisor_register_post(void) { return 0; }
